...
async def text_message(message: types.Message):
    if message.text == 'Отследить заказ':
        try:
            ...
            if track == []:
                ...
                for i in range(len(track)):
                    mes=track[i]
                    await message.answer(f'Номер накладной: {mes[0]} \n Статус: {mes[1]} \n Дата получения: {mes[2]}')
        except:
            markup = types.ReplyKeyboardMarkup()
            markup.add(types.KeyboardButton("Авторизоваться"))
        ...
        if islogin(message.chat.id) == True:
            await message.answer('Перевод на менеджера, напишите ваш вопрос')
            await zap.close.set()
        else:
            markup = types.ReplyKeyboardMarkup()
            markup.add(types.KeyboardButton("Авторизоваться"))
...
async def help(message: types.Message, state: FSMContext):
    ...
    if message.text != '/close' or message.text != '/start':
        cursor = connection.cursor()
...
async def get_address1(message: types.Message, state: FSMContext):
...
async def get_address2(message: types.Message, state: FSMContext):
...
async def get_payment_method(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        connection.commit()
        cursor.execute(f"Insert into tracking (ContractID, InvoiceID, Type) VALUES ('{login[0]}', {recep}, 'Сформировано');")
        connection.commit()
        cursor.close()
        markup = types.ReplyKeyboardMarkup()
        but1 = types.KeyboardButton("Создать накладную")
        ...
        but4 = types.KeyboardButton("Перевод на менеджера")
        markup.add(but1, but2, but3, but4)
        await message.answer('успех', reply_markup=markup)
    except:
        markup = types.ReplyKeyboardMarkup()
        markup.add(types.KeyboardButton("Авторизоваться"))
        await message.answer('Вы не вошли в учётную запись', reply_markup=markup)
